...
# Lendo os dados
df = pd.read_csv("prova_2\dados_formatados_novos.csv", sep= "\t")
...
estacoes_nao_utilizadas = ["41151000", "41250000"]
...
for col in df.columns:
    ...
    if col in estacoes_nao_utilizadas:
        plt.scatter(Tr, col_data, marker="x", label=col+"*")
    else:
...
log_norm3 = LogNorm3ParamHosking(qsi=0.9157, alfa=0.2848, kapa=-0.1737)
log_norm3_CPRM = LogNorm3ParamHosking(qsi=0.929, alfa=0.448, kapa=-0.311)

tempos = [1.01, 2, 5, 10, 20, 25, 50, 100, 150, 200]
for n, i in enumerate(["TR", "GEV", "Log-Normal 3P (MML)", "Log-Normal 3P (CPRM)"]):
...
plt.plot(Tr, y_ln3, label="Log-Normal 3P (MML)")
...
plt.savefig(fr"F:\Mestrado\1° Semestre\Hidrologia estatística\P2\comparacao_logn3_ex2.svg")
...
for estacao in df.columns:
    ...
    y_now = q_med*y_ln3
    plt.plot(Tr, y_now, label="Log-Normal 3P (MML)")
    ...
    plt.savefig(f'F:\Mestrado/1° Semestre/Hidrologia estatística/P2/quantis_novos/{estacao}.svg')
